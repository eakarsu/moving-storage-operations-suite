const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5440';
...
const adminCookie = await login('admin@moving-storage-operations.local', 'admin123');
const managerCookie = await login('manager@moving-storage-operations.local', 'manager123');
const analystCookie = await login('analyst@moving-storage-operations.local', 'analyst123');
